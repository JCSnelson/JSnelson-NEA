...
def add_item(item_id: file_path, amount):
   if count_stored_items(save_id) > max_inventory_size: #Full Inventory
      ...
      #Checks if you can stack the item and either adds a new entry or stacks it
         if get_stored_item_amount(save_id,item_id) and item.stackable: #If the item is in the database and stackable
...
\section{Development}
        ...
        I added functions to the database script in order to utilise the current\_save\_id script variable so that I dont have to input the variable every time I want to run a save\_data or stored\_items query. The functions also return the query result.\\
