...
def _ready():
   if Slimes:
	 ...
	 slime.damage = floor(Global.current_level*log(3*Global.difficulty))
        ...
        \end{tabular}
        \textbf{Successfull:}\\
        The Dungeon Environment and different rooms can be seen through the procedural generation tests here$^{reference}$. The Doors and Chests can be seen through their tests here$^{reference}$
        ...
        \begin{tabular}{|c|c|c|c|}
                ...
                \hline
        \end{tabular}
        All the UI elements where implemented using godot's CanvasLayer so that they dont move when the camera moves. The UI is evidenced through the video testing here$^{[4]}$\\
...
\begin{tabular}{|c|c|c|c|}
        ...
        \hline
        1&&Movement Testing&\\
        2&&Player Attack System and Enemies that damage the player&\\
